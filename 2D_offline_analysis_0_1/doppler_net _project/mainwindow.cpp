...
unsigned char buf;
int tag0=1,tag1=2,tag2=3,tag3=4;//tag1\tag2用于区别强度值0的来源，即建立两套平行的坐标系，一套用于标记，一套用于存放强度值画图,tag3用于记录该点平滑次数
...
//                qDebug()<<"插值前"<<colorMap->data()->cell(xIndex,yIndex);
                ...
                map2[xIndex][yIndex]=tag2;//tag2表示是插值得到
...
//                qDebug()<<"------";
            ...
            if((map2[xIndex][yIndex]==tag2)&&(colorMap->data()->cell(xIndex,yIndex)==0))//筛选出进过插值仍然为0的带插值点
               {
                ...
                colorMap->data()->setCell(xIndex, yIndex,val);
                map2[xIndex][yIndex]=tag3;//tag3表示此点已经经过一次插值后的平滑处理
       ...
       connect(ui->angle_spinBox,SIGNAL(valueChanged(int)),this,SLOT(angleupdate()));//监测角度选择框的数值变化
