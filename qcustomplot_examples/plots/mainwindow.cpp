  ...
  ui(new Ui::MainWindow)
{
  ...
  // 0:  setupQuadraticDemo(ui->customPlot);二次方程
  // 1:  setupSimpleDemo(ui->customPlot);指数函数画图+缩放，修改后出错了！
  // 2:  setupSincScatterDemo(ui->customPlot);sin函数+置信区间
  // 3:  setupScatterStyleDemo(ui->customPlot);展示散点的不同样式，内置的样式
  // 4:  setupScatterPixmapDemo(ui->customPlot);用外置的图像作为散点
  // 5:  setupLineStyleDemo(ui->customPlot);各种线的样式
  // 6:  setupDateDemo(ui->customPlot);
  // 7:  setupTextureBrushDemo(ui->customPlot);以外部图像作为填充
  // 8:  setupMultiAxisDemo(ui->customPlot);在一张图中显示多种图表
  // 9:  setupLogarithmicDemo(ui->customPlot);对数
  // 10: setupRealtimeDataDemo(ui->customPlot);实时波形!!!
  // 11: setupParametricCurveDemo(ui->customPlot);参数曲线+鼠标动作的交互
  // 12: setupBarChartDemo(ui->customPlot);柱状图绘制
  ...
  // 17: setupAdvancedAxesDemo(ui->customPlot);轴的设置+一个窗口显示多张图
  // 18: setupColorMapDemo(ui->customPlot);彩图，强度场！！！
  // 19: setupFinancialDemo(ui->customPlot);股票趋势
...
void MainWindow::setupColorMapDemo(QCustomPlot *customPlot)
{
  ...
  colorMap->data()->setRange(QCPRange(-8, 8), QCPRange(-8, 8)); // and span the coordinate range -4..4 in both key (x) and value (y) dimensions
